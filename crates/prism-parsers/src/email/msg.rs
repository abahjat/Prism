...
impl MsgParser {
    ...
    fn extract_string_property(&self, comp: &mut CompoundFile<Cursor<&[u8]>>, prop_path: &str) -> Option<String> {
        comp.open_stream(prop_path)
            .ok()
            .and_then(|mut stream| {
                use std::io::Read;
...
impl Parser for MsgParser {
    ...
    async fn parse(&self, data: Bytes, context: ParseContext) -> Result<Document> {
        ...
        } else if let Some(sender_email) = self.extract_string_property(&mut comp, "__substg1.0_0C1F001F") {
...
".to_string(),
        ...
        let body_text = if let Some(body) = self.extract_string_property(&mut comp, "__substg1.0_1000001F") {
            body
        } else if let Some(body) = self.extract_string_property(&mut comp, "__substg1.0_10130102") {
            // HTML body (0x1013, 0102 = binary)
            body
        } else {
